...
```{r scaler estimation from Klug-Baerwald}
...
```{r Algorithm for expected length and other measures}
################################################################################
...
#forloop to run through each torpot bout
...
```{r running function for model description}
...
```{r calculate torpor bout length based on average temp/WVP per bout}
...
```{r testing that average estimates do not fit the measured data better}
...
for(i in 1:4){
  g[i] ~ dnorm(0,1/10) # Uninformative priors to determine rho
}
sigma.rho ~ dunif(0,0.5)

#Estimate population values using literature priors
mu.C.t ~ dunif(0.01,0.2)
sigma.C.t ~ dunif(0.001,0.05)
mu.rEWL.body ~ dgamma(0.1^2/0.1^2,0.1/0.1^2)
sigma.rEWL.body ~ dunif(0.001,0.1)
mu.t.tor.max ~ dunif(max.bout,1000)
sigma.t.tor.max ~ dunif(0.001,100)
mu.T.tor.min ~ dunif(0.5,5)
sigma.T.tor.min ~ dunif(0.001,1)
mu.TMR.min ~ dunif(0.01,0.2)
sigma.TMR.min ~ dunif(0.001,0.1)
...
for(i in 1:n.bats){
  ...
  t.tor.max[i] ~ dgamma(mu.t.tor.max^2/sigma.t.tor.max^2,mu.t.tor.max/sigma.t.tor.max^2)
  T.tor.min[i] ~ dgamma(mu.T.tor.min^2/sigma.T.tor.min^2,mu.T.tor.min/sigma.T.tor.min^2)
...
for(j in 1:n.bouts[1]){
    ...
    a[j] <- exp(g[1]+g[2]*T.a.scale[j]+g[3]*d.WVP[j]+g[4]*T.a.scale[j]*d.WVP[j])/ (1+exp(g[1]+g[2]*T.a[j]+g[3]*d.WVP[j]+g[4]*T.a.scale[j]*d.WVP[j]))
...
for(i in 2:n.bats){
  for(j in 1:n.bouts[i]){
    ...
    a[sum(n.bouts[1:(i-1)])+j] <- exp(g[1]+g[2]*T.a.scale[sum(n.bouts[1:(i-1)])+j]+g[3]*d.WVP[sum(n.bouts[1:(i-1)])+j]+g[4]*T.a.scale[sum(n.bouts[1:(i-1)])+j]*d.WVP[sum(n.bouts[1:(i-1)])+j])/ (1+exp(g[1]+g[2]*T.a[sum(n.bouts[1:(i-1)])+j]+g[3]*d.WVP[sum(n.bouts[1:(i-1)])+j]+g[4]*T.a.scale[sum(n.bouts[1:(i-1)])+j]*d.WVP[sum(n.bouts[1:(i-1)])+j]))
...
zm.Ens.ind=coda.samples(jm.Ens.ind, variable.names=c("sigma",
                                             ...
                                             "sigma.T.tor.min",
                                             "mu.TMR.min",
                                             "sigma.TMR.min"),n.iter=n.iter,n.thin=10)
...
zj.Ens.ind=jags.samples(jm.Ens.ind, variable.names=c("sigma",
                                             ...
                                             "bout.length.pop",
                                             "mean.pop",
                                             "var.pop"),n.iter=n.iter,n.thin=10)

plot(zm.Ens.ind)
summary(zm.Ens.ind)
parm.Ens <- as.data.frame(do.call(rbind,zm.Ens.ind))
write.csv2(x = as.data.frame(parm.Ens), file = "EnsembleGuidedUmbrellaPopulationToIndividual.csv")
...
#Calculate model expectations given original parameters
  ...
  Haase.model[i] <- (t.cool+min(mu.bout.length.WVP.mu, mu.bout.length.Hayman)+t.warm)/24
...
d.WVP.model[sum(n.bouts[1:(i-1)])+j] <- max(.611*exp((17.503*max(df.Ens$T.a[sum(n.bouts[1:(i-1)])+j],T.tor.min[i]))/(max(df.Ens$T.a[sum(n.bouts[1:(i-1)])+j],T.tor.min[i])+240.97))-df.Ens$WVP.a[sum(n.bouts[1:(i-1)])+j],0.001)
  ...
  Haase.model[sum(n.bouts[1:(i-1)])+j] <- (t.cool+min(mu.bout.length.WVP.mu, mu.bout.length.Hayman)+t.warm)/24
}
}
...
```{r compare parameters for all bats and three different bats}
library(cowplot)

df.parm <- rbind(c(summary(zj.Ens.ind$mu.C.t, quantile, c(0.025,0.5,0.975))$stat, "All bats", "Torpor conductance\n(ml O2/gC)","Population Model"),
                 ...
                 c(summary(zj.Ens.ind$mu.T.tor.min, quantile, c(0.025,0.5,0.975))$stat, "All bats", "Min. torpor temp.\n(C)","Population Model"),
                 ...
                 c(summary(zj.Ens.ind$mu.t.tor.max, quantile, c(0.025,0.5,0.975))$stat/24, "All bats", "Max. torpor length\n(Days)","Population Model"),
                 ...
                 c(summary(zj.Ens.ind$mu.rEWL.body, quantile, c(0.025,0.5,0.975))$stat, "All bats", "Body evap. loss rate\n(mg/[hr*d.WVP*cm2])","Population Model"),
                 ...
                 c(summary(zj.Ens.ind$mu.TMR.min, quantile, c(0.025,0.5,0.975))$stat, "All bats", "Min. metabolic rate\n(mg O2/[g*hour])","Population Model"),
                 ...
                 c(NA, 0.1, NA, "Lit.", "Body evap. loss rate\n(mg/[hr*d.WVP*cm2])","Literature"),
...
df.moments.indmodels<- rbind(c(mean(df$bout.length),confint(lm(df$bout.length~1),level=0.95),"Mean","All bats","Data"),
                   ...
                   c(var(df$bout.length),"NA", "NA","Variance","All bats","Data"),
                   ...
                   c(summary(zj.Ens.ind$mean.bat,mean)$stat[1], t(summary(zj.Ens.ind$mean.bat, quantile, c(0.025,0.975))$stat)[1,], "Mean","Bat1","Individual Parameters"),
                   c(summary(zj.Ens.ind$mean.bat,mean)$stat[2], t(summary(zj.Ens.ind$mean.bat, quantile, c(0.025,0.975))$stat)[2,], "Mean","Bat2","Individual Parameters"),
                   c(summary(zj.Ens.ind$mean.bat,mean)$stat[3], t(summary(zj.Ens.ind$mean.bat, quantile, c(0.025,0.975))$stat)[3,], "Mean","Bat3","Individual Parameters"),
                   c(summary(zj.Ens.ind$var.bat,median)$stat[1], t(summary(zj.Ens.ind$var.bat, quantile, c(0.025,0.975))$stat)[1,], "Variance","Bat1","Individual Parameters"),
                   c(summary(zj.Ens.ind$var.bat,median)$stat[2], t(summary(zj.Ens.ind$var.bat, quantile, c(0.025,0.975))$stat)[2,], "Variance","Bat2","Individual Parameters"),
                   c(summary(zj.Ens.ind$var.bat,median)$stat[3], t(summary(zj.Ens.ind$var.bat, quantile, c(0.025,0.975))$stat)[3,], "Variance","Bat3","Individual Parameters"),
                   c(summary(zj.Ens.ind$t.hib,mean)$stat[1], t(summary(zj.Ens.ind$t.hib, quantile, c(0.025,0.975))$stat)[1,], "t.hibernation","Bat1","Individual Parameters"),
                   c(summary(zj.Ens.ind$t.hib,mean)$stat[2], t(summary(zj.Ens.ind$t.hib, quantile, c(0.025,0.975))$stat)[2,], "t.hibernation","Bat2","Individual Parameters"),
                   c(summary(zj.Ens.ind$t.hib,mean)$stat[3], t(summary(zj.Ens.ind$t.hib, quantile, c(0.025,0.975))$stat)[3,], "t.hibernation","Bat3","Individual Parameters"),
                   c(summary(zj.Ens.ind$mean.pop,mean)$stat, t(summary(zj.Ens.ind$mean.pop, quantile, c(0.025,0.975))$stat), "Mean","All bats","Shared Parameters"),
                   c(summary(zj.Ens.ind$var.pop,mean)$stat, t(summary(zj.Ens.ind$var.pop, quantile, c(0.025,0.975))$stat), "Variance","All bats","Shared Parameters"))  
